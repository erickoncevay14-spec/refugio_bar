    ...
    public String procesarLogin(@RequestParam String usuario, @RequestParam String password, Model model) {
        ...
        if (userOpt.isPresent()) {
            ...
            if (user.getActivo() == null || !user.getActivo()) {
                model.addAttribute("error", "Usuario inhabilitado. Contacte al administrador.");
                return "pagina/login";
            }
...
public ResponseEntity<Usuario> cambiarRolAjax(@RequestParam Long usuarioId, @RequestParam String nuevoRol) {
    ...
    public ResponseEntity<List<Usuario>> obtenerUsuarios(
            @org.springframework.web.bind.annotation.RequestParam(required = false) String rol,
            @org.springframework.web.bind.annotation.RequestParam(required = false) String sinRol
    ) {
        List<Usuario> usuarios;
        if (rol != null && !rol.isEmpty()) {
            usuarios = usuarioRepository.findByRol_NombreIgnoreCase(rol);
        } else if (sinRol != null && !sinRol.isEmpty()) {
            usuarios = usuarioRepository.findByRol_NombreIgnoreCaseNot(sinRol);
        } else {
            usuarios = usuarioRepository.findAll(Sort.by("id"));
        }
