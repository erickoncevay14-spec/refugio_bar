...
public class AuthController {
    ...
    public ResponseEntity<?> login(@Valid @RequestBody LoginRequest loginRequest) {
        try {
            // Obtener usuario y validar si está activo
            Usuario usuario = authService.findByEmail(loginRequest.getEmail());
            if (usuario == null || usuario.getActivo() == null || !usuario.getActivo()) {
                return ResponseEntity.status(403).body("Usuario desactivado. Contacte al administrador.");
            }
            // Autenticar con Spring Security usando email
            ...
            response.setUsername(usuario.getEmail());
    ...
    public ResponseEntity<?> register(@Valid @RequestBody RegisterRequest registerRequest) {
        try {
            ...
            AuthResponse response = new AuthResponse();
            response.setToken(jwt);
            response.setUsername(nuevoUsuario.getEmail());
